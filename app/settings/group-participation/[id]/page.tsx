...
import type { GroupProviderPlanParticipationDetailDto } from "@/lib/services/groupParticipations";
import type { EntityProviderLookupDto, PlanLookupDto, ValueLabelDto } from "@/lib/services/lookups";

const MODULE_NAME = "Group Provider Plan Participations";
...
export default function GroupParticipationDetailPage() {
  ...
  const [participation, setParticipation] = useState<GroupProviderPlanParticipationDetailDto | null>(null);
  ...
  const fetchParticipation = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const detail = await groupParticipationsApi().getById(id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await groupParticipationsApi().update(id, {
  ...
  if (error || !participation) {
    return (
      <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 px-6">
        ...
        <Button onClick={() => router.push("/settings/group-participation")} className="h-10 rounded-[5px] bg-[#0066CC] px-[18px] text-white hover:bg-[#0066CC]/90 font-aileron text-[14px]">
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col px-6">
      ...
      <nav className="-mx-6 mb-5 flex items-center gap-2 bg-[#F7F8F9] px-6 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
        ...
        <Link href="/settings/group-participation" className="transition-colors hover:text-foreground">Provider-Plan Participation</Link>
        <span aria-hidden>/</span>
